...
#include "log.h"
#include "proceso.h"    /* Fase 2: gestión de procesos */
...
void command_help() {
  ...
  printf("  apagar               : Salir del simulador (alias de exit)\n");
  printf("  ps                   : Ver tabla de procesos\n");
  printf("  load <archivo>       : Cargar un programa (sin ejecutar)\n");
  printf("  run                  : Ejecutar procesos en tabla\n");
...
void command_run() {
  ...
  if (hay_procesos) {
    ...
    printf("Ejecutando con planificador Round-Robin (quantum=%d)...\n",
           PROCESS_QUANTUM);
    printf("Presione Ctrl+C en la terminal si desea abortar.\n");
    int max_cycles = 5000; /* Reducido para evitar loops infinitos molestos */
    int cycles = 0;
    int idle_count = 0;
...
int main() {
  ...
  while (1) {
    ...
    if (strcmp(cmd, "exit") == 0 || strcmp(cmd, "apagar") == 0) {
      ...
      inicializarCPU();
      process_init();
      printf("Sistema reiniciado (CPU, Pila SO y Particiones de Procesos).\n");
